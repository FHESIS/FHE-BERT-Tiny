...
# The tokenizer/model files used below are already cached locally after the
# first run; skip the Hub etag round-trip on every subsequent invocation
# (saves ~0.7s/process). Only takes effect if the caller hasn't already set it.
...
def load_model():
    """Load the tokenizer/model once. Reuse the returned objects across many
    classify() calls instead of re-importing/reloading per sentence -- the
    import + load cost (~8s) otherwise dwarfs the ~0.15s inference itself."""
...
@torch.no_grad()
def classify(tokenizer, model, device, sentence):
    """Run the precomputed-LayerNorm plaintext circuit (mirrors the FHE
    circuit's approximations) and, separately, the real PyTorch model, on a
    single sentence. Returns precomputed/PyTorch logits and predictions."""
    ...
    seq_len = fin2_backup.size(1)
    mean_t = torch.from_numpy(mean[:seq_len]).to(dtype=fin2_backup.dtype, device=fin2_backup.device).view(1, seq_len, 1)
    var_t = torch.from_numpy(var[:seq_len]).to(dtype=fin2_backup.dtype, device=fin2_backup.device).view(1, seq_len, 1)
    ...
    fin3_whole = (fin2_backup - mean_t) * var_t * w_output_layernorm + b_output_layernorm
    ...
    seq_len = fin_6.size(1)
    mean_t = torch.from_numpy(mean[:seq_len]).to(dtype=fin_6.dtype, device=fin_6.device).view(1, seq_len, 1)
    var_t = torch.from_numpy(var[:seq_len]).to(dtype=fin_6.dtype, device=fin_6.device).view(1, seq_len, 1)

    w_output_layernorm = model.bert.encoder.layer[0].output.LayerNorm.weight.clone().detach().double()
    b_output_layernorm = model.bert.encoder.layer[0].output.LayerNorm.bias.clone().detach().double()

    fin7_whole = (fin_6 - mean_t) * var_t * w_output_layernorm + b_output_layernorm
    ...
    w_output_layernorm = model.bert.encoder.layer[1].attention.output.LayerNorm.weight.clone().detach().double()
    ...
    w_output_layernorm = model.bert.encoder.layer[1].output.LayerNorm.weight.clone().detach().double()
    ...
    return {
        "precomputed_logits": output.detach().cpu().numpy(),
        "pytorch_logits": output_real.detach().cpu().numpy(),
    }


def sentiment(logits):
    return "negative" if logits[0] > logits[1] else "positive"


def _print_result(result):
    print(f"Plain-Precomputed: {sentiment(result['precomputed_logits'])} sentiment!")
    print(f"Plain-PyTorch    : {sentiment(result['pytorch_logits'])} sentiment!")
    print("Output logits Plain-Precomputed: " + str(result["precomputed_logits"]))


if __name__ == "__main__":
    tokenizer, model, device = load_model()

    # Single-sentence CLI usage (one argv per invocation) is unchanged for
    # backward compatibility with existing callers. Passing more than one
    # sentence as separate argv reuses the already-loaded model for all of
    # them instead of paying import/load cost again per sentence.
    for sentence in sys.argv[1:]:
        _print_result(classify(tokenizer, model, device, sentence))
